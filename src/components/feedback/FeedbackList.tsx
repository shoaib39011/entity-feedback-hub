...
import { Feedback, FeedbackCategory } from "@/context/FeedbackContext";
...
const FeedbackList = ({ feedbacks, showResponse = true }: FeedbackListProps) => {
    ...
    <div className="space-y-4">
      {feedbacks.map((feedback) => (
        <Card key={feedback.id} className="overflow-hidden">
          <div className={`h-2 ${
            feedback.category === "complaint" ? "bg-feedback-complaint" :
            feedback.category === "suggestion" ? "bg-feedback-suggestion" :
            "bg-feedback-compliment"
          }`} />
          <CardHeader>
